  ...
  try {
    if (event.event === 'ingress_started') {
      if (!event.ingressInfo) {
        return new Response(
          "No ingress info available on 'ingress_ended' event.",
          { status: 400 }
        );
      }
      await db.execute(
        'UPDATE stream SET is_live = true WHERE ingress_id = ?',
      ...
      await db.execute(
        'UPDATE stream SET is_live = false WHERE ingress_id = ?',
        [event.ingressInfo.ingressId]
      );

      // delete the chats associated with the stream

      const [results, _] = await db.execute<StreamResult[]>(
        'SELECT username FROM stream JOIN tc_user ON stream.user_id = tc_user.id WHERE ingress_id = ?;',
        [event.ingressInfo.ingressId]
      );
      ...
      console.log(`Ingress API ignoring ${event.event}.`)
    }
  } catch (err) {
    console.log(err);
