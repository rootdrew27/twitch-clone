...
export const getUserWithFollowerCount = async (username: string) => {
    ...
    try {
        ...
        const [userResults, _] = await db.execute<UserResult[]>("SELECT tc_user.id AS id, username, is_live, bio, clerk_id FROM tc_user JOIN stream ON tc_user.id = stream.user_id WHERE username = ?;", [username]);

        const user = userResults[0];

        const [followerCountResults, _fields] = await db.execute<RowDataPacket[]>("SELECT COUNT(*) AS follower_count FROM follow WHERE following_id = ?;", [user.id]);
