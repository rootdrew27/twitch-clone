...
export const updateStream = async ({ field, value }: StreamConfig) => {
  ...
  try {
    ...
    if (field === 'is_chat_enabled') {
      await db.execute(
        'UPDATE stream SET is_chat_enabled = ? WHERE user_id = ?;',
        values
      );
    } else if (field === 'is_chat_delayed') {
      await db.execute(
        'UPDATE stream SET is_chat_delayed = ? WHERE user_id = ?;',
        values
      );
    } else if (field === 'is_chat_followers_only') {
      await db.execute(
        'UPDATE stream SET is_chat_followers_only = ? WHERE user_id = ?;',
        values
      );
    } else if (field == 'name') {
      ...
      throw new Error(
        'Internal Error: The field parameter does not match a valid case.'
      );
    }
